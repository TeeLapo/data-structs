...
char* pawn_move(char board[SIZE][SIZE], int r, int c, int new_r, int new_c, bool user) {
    ...
    if (abs(new_r - r) > 1)
        if (r == 6 && user && c == new_c || r == 1 && !user && c == new_c){
        }
        else
            return NULL;

    //Validate there is no piece in the way of a straight move
    if(c == new_c){
    ...
    //Check for promotion
    if (user && new_r == 0) {
        printf("Your pawn has been promoted. Please select which piece you would like to promote to (q, r, b, n):\n");
        scanf(" %c", &piece);
    }
    if (!user && new_r == 7) {
        printf("Your pawn has been promoted. Please select which piece you would like to promote to (Q, R, B, N):\n");
    ...
    //Ensure move is only one step in any direction
    if (abs(new_r - r) > 1 || abs(new_c - c) > 1)
        return NULL;

    //Check rook move for king
    board[r][c] = rook_piece;
    char* moved_piece = rook_move(board, r, c, new_r, new_c, user);
    if (moved_piece != NULL) { //Success
